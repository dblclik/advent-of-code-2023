...
class CubeGame:
    ...
    def validate_game(self, game_string):
        ...
        invalid_green = [green for green in matches.get("green") if int(green) > self.green_limit]
        if len(invalid_green) > 0:
            return False
        
        
        invalid_blue = [blue for blue in matches.get("blue") if int(blue) > self.blue_limit]
